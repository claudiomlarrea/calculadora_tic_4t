...
# crear versión que acepte int y str
ETIQUETAS_BINARIAS_STR = {
    var: ({**m, **{str(k): v for k, v in m.items()}}) for var, m in ETIQUETAS_BINARIAS.items()
}
...
ETIQUETAS_EDUC_STR = {**ETIQUETAS_EDUC, **{str(k): v for k, v in ETIQUETAS_EDUC.items()}}
...
def _edad_grupos(df: pd.DataFrame) -> pd.Series:
    ...
    if c and pd.api.types.is_numeric_dtype(df[c]):
        ...
        s = pd.cut(e, bins=bins, labels=labels)
        return pd.Series(s, name="Edad")
    # Fallback si no hay edad
    s = pd.qcut(range(len(df)), 5, labels=["0-17", "18-29", "30-44", "45-64", "65+"])
    return pd.Series(s, name="Edad")
...
def _quintiles_ingreso(df: pd.DataFrame) -> Optional[pd.Series]:
    ...
    try:
        ...
        return pd.Series(q, name="Quintil ingreso")
    except Exception:
        return None

def _map_nominal(nombre: str, s: pd.Series) -> pd.Series:
    # normalizar a string por si vienen int/str mezclados
    s = pd.Series(s, copy=False)
    if nombre in ETIQUETAS_BINARIAS:
        m = ETIQUETAS_BINARIAS_STR[nombre]
        s = s.astype(str).str.strip().replace({"SI":"Sí","Si":"Sí","NO":"No","N0":"No"})
        return s.map(m).fillna(s)
    if nombre in CANDIDATAS_EDUC:
        ser = s.astype(str).map(ETIQUETAS_EDUC_STR).fillna(s)
        return ser
    return s

def _value_counts_w(s: pd.Series, w: Optional[pd.Series]) -> pd.DataFrame:
    # todo como texto para evitar comparaciones int<->str
    s = pd.Series(s, copy=False).astype("string")
    if w is not None and len(w) == len(s):
        tmp = pd.DataFrame({"x": s, "w": w}).dropna(subset=["x"])
        out = tmp.groupby("x", as_index=False)["w"].sum().rename(columns={"x": s.name or "valor", "w": "Cantidad"})
        out["Cantidad"] = out["Cantidad"].round(0).astype(int)
    else:
        vc = s.value_counts(dropna=False).reset_index()
        vc.columns = [s.name or "valor", "Cantidad"]
        out = vc
    # ordenar por representación de texto
    key_col = out.columns[0]
    out = out.sort_values(by=key_col, key=lambda col: col.astype(str)).reset_index(drop=True)
    return out

def _prop_w(flag: pd.Series, w: Optional[pd.Series]) -> float:
    flag = pd.Series(flag, copy=False).astype(float)
    if w is not None and len(w) == len(flag):
        df = pd.DataFrame({"f": flag, "w": w}).dropna(subset=["f"])
        num = (df["f"] * df["w"]).sum()
        den = df["w"].sum()
        return float(num/den) if den > 0 else 0.0
    return float(flag.mean()) if len(flag) else 0.0

def _ensure_series_with_name(x, default_name="Grupo") -> pd.Series:
    if isinstance(x, pd.Series):
        return x if x.name else x.rename(default_name)
    # ExtensionArray / ndarray / list
    return pd.Series(x, name=default_name)

def _tabla_prop_por(g, flag: pd.Series, w: Optional[pd.Series]) -> pd.DataFrame:
    g = _ensure_series_with_name(g, "Grupo")
    flag = pd.Series(flag, copy=False).astype(int)
    data = pd.DataFrame({"g": g, "f": flag})
    data = data.dropna(subset=["g"])
    if w is not None and len(w) == len(g):
        data["w"] = w
        agg = data.groupby("g", dropna=False).apply(lambda d: (d["f"]*d["w"]).sum() / d["w"].sum()).reset_index(name="Porcentaje")
    else:
        agg = data.groupby("g", dropna=False)["f"].mean().reset_index(name="Porcentaje")
    agg = agg.rename(columns={"g": g.name})
...
def generar_analisis_tic_ampliado(df_merged: pd.DataFrame) -> Tuple[Dict[str, pd.DataFrame], pd.DataFrame, Dict[str, Any]]:
    ...
    # Indicadores de exclusión - comparar como string
    ip4 = df["IP_III_04"].astype(str)
    ip6 = df["IP_III_06"].astype(str)
    df["excluido_binario"] = ((ip4 == "2") & (ip6 == "2")).astype(int)
    df["exclusion_ordinal"] = ((ip4 == "2") & (ip6 == "2")).astype(int) * 2 + \
                              (((ip4 == "2") ^ (ip6 == "2")).astype(int) * 1)

    # Etiquetas para listados
    for c in ["IP_III_04","IP_III_05","IP_III_06","IH_II_01","IH_II_02","CH04"]:
        if c in df.columns:
            df[c] = _map_nominal(c, df[c])

    edad = _edad_grupos(df)
    ...
    excl_ordinal["Nivel"] = excl_ordinal[excl_ordinal.columns[0]].map({0:"Sin exclusión",1:"Exclusión parcial",2:"Exclusión total"})
    ...
    if ("IH_II_02" in df.columns) and (quintil is not None):
        acc_int = df["IH_II_02"].astype(str).isin(["Sí","1"])
        acc_quintil = _tabla_prop_por(quintil, acc_int.rename("acceso"), w)
        acc_quintil = acc_quintil.rename(columns={"Porcentaje":"% hogares con internet"})
        tablas["Acceso a internet por quintil"] = acc_quintil

    # Segmentos críticos (Edad × Educación × Sexo) – top 5 tasas
    ...
    if keys:
        if "w" in segs.columns:
            top = segs.dropna(subset=keys).groupby(keys, dropna=False).apply(lambda d: (d["ex"]*d["w"]).sum()/d["w"].sum()).reset_index(name="Tasa exclusión (%)")
        else:
            top = segs.dropna(subset=keys).groupby(keys, dropna=False)["ex"].mean().reset_index(name="Tasa exclusión (%)")
    ...
    if "IH_II_01" in df.columns:
        resumen["hogar_con_pc_pct"] = round(_prop_w(df["IH_II_01"].astype(str).isin(["Sí","1"]), w)*100, 2)
    if "IH_II_02" in df.columns:
        resumen["hogar_con_internet_pct"] = round(_prop_w(df["IH_II_02"].astype(str).isin(["Sí","1"]), w)*100, 2)
...
def generar_informe_narrativo_tic(tablas: Dict[str, pd.DataFrame], anio: str = "2024", resumen: Optional[Dict[str, Any]] = None) -> BytesIO:
    ...
    if resumen:
        ...
        if "edad_brecha_pp" in resumen and "edad_peak" in resumen and "edad_valley" in resumen:
            bullets.append(f"Por edad, la brecha entre el grupo más afectado ({resumen['edad_peak'].get('Edad','')}: {resumen['edad_peak'].get('Porcentaje','')}%) y el menos afectado ({resumen['edad_valley'].get('Edad','')}: {resumen['edad_valley'].get('Porcentaje','')}%) es de {resumen['edad_brecha_pp']} p.p.")
        if "sexo_brecha_pp" in resumen:
            bullets.append(f"Se observa una diferencia por sexo de {resumen['sexo_brecha_pp']} p.p. en la tasa de exclusión.")
        if "quintil_brecha_pp" in resumen and "quintil_low" in resumen and "quintil_high" in resumen:
            bullets.append(f"Por ingreso del hogar, el acceso a internet muestra {resumen['quintil_brecha_pp']} p.p. de diferencia entre {resumen['quintil_low'].get('Quintil ingreso','Q1')} ({resumen['quintil_low'].get('% hogares con internet','')}%) y {resumen['quintil_high'].get('Quintil ingreso','Q5')} ({resumen['quintil_high'].get('% hogares con internet','')}%).")
    ...
    _p(doc, "• Expandir conectividad fija de calidad y planes de asequibilidad en territorios rezagados.")
    _p(doc, "• Programas de acceso a computadoras para hogares Q1–Q2 con estudiantes.")
    _p(doc, "• Alfabetización digital segmentada por edad y nivel educativo, con tutorías prácticas.")
    _p(doc, "• Promover usos con retorno social: educación, empleo y trámites digitales.")

    # 5. Limitaciones
    doc.add_heading("5. Limitaciones y próximas acciones", level=1)
    _p(doc, "Los resultados dependen de la disponibilidad de variables por trimestre. Complementar con análisis multivariados y comparativas interanuales 2017–2024.")
